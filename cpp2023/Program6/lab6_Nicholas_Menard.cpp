...
/* Nicholas Menard
/* Lab 6 Solution                                                    
/* This program gives you options to read, delete and display tracks in a collection 
...
int mainMenuOption()
{
    ...
    cout << "6. Add Tracks for a CD" << endl;
    cout << "7. Delete All Tracks for a CD" << endl;
...
void addTracksMenu(CD findaCD[], int count)
{
    ...
    {
        addTracks(findaCD[i]);
        cout << "Tracks added for CD: " << title << endl;
    }
    else
        cout << title << " is not in the collection" << endl;
}
/**************************************************/
/* Function:   addTracks                                       
/* Inputs:     cd - CD object
...
void deleteAllTracks(CD allCDs[], int count)
{
    ...
    // Find the CD in the collection
    while (i < count && !found)
    {
        if (title == allCDs[i].getTitle() && allCDs[i].getNumTracks() > 0)
            found = true;
        else
            i++;
    }
    ...
    if (found)
    {
        allCDs[i].deleteTracks();
        cout << "All tracks deleted for CD: " << title << endl;
    }
    else
    {
        cout << "CD with title \"" << title << "\" not found or has no tracks." << endl;
    }
}
